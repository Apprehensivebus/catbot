...
public class Perms {
    ...
    public static HashMap<Long, BitSet> parsePerms(JSONObject perms){
        ...
        for (String key : perms.keySet()){
            permSet.clear();
            parsedPerms.put(Long.parseLong(key),(BitSet) perms.get(key));
        }
        return parsedPerms;
    }

    public static JSONObject initPerms(){ //creates the permission bitset for each channel
        ...
        homeperms.set(1);// enable second perm by default for now
    ...
    public static JSONObject loadPerms(){ //creates the permission bitset for each channel
        ...
        try {
            ...
            System.out.println(perms.get(String.valueOf(homechannel)));
            return perms;
        } catch (Exception e) {
