...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        DiscordApi api = new DiscordApiBuilder().setToken(token).addIntents(Intent.MESSAGE_CONTENT).addIntents(Intent.GUILD_MEMBERS).login().join();
        ...
        api.addSlashCommandCreateListener(event -> {
            SlashCommandInteraction slashCommandInteraction = event.getSlashCommandInteraction();
            if (slashCommandInteraction.getFullCommandName().equals("catgirlbotweirb weirb add")) {
                if (slashCommandInteraction.getUser().isBotOwner() || slashCommandInteraction.getServer().get().hasPermission(slashCommandInteraction.getUser(), PermissionType.valueOf("MANAGE_CHANNELS"))) {
            ...
            if (slashCommandInteraction.getFullCommandName().equals("catgirlbotweirb2 weirb test")) {
                if (slashCommandInteraction.getUser().isBotOwner() || slashCommandInteraction.getServer().get().hasPermission(slashCommandInteraction.getUser(), PermissionType.valueOf("MANAGE_CHANNELS"))) {
                    Server server = slashCommandInteraction.getServer().get();
            ...
            if (slashCommandInteraction.getFullCommandName().equals("catgirlbotbad bad add")) {
                if (slashCommandInteraction.getUser().isBotOwner() || slashCommandInteraction.getServer().get().hasPermission(slashCommandInteraction.getUser(), PermissionType.valueOf("MANAGE_CHANNELS"))) {
            ...
            if (slashCommandInteraction.getFullCommandName().equals("catgirlbotbad2 bad test")) {
                if (slashCommandInteraction.getUser().isBotOwner() || slashCommandInteraction.getServer().get().hasPermission(slashCommandInteraction.getUser(), PermissionType.valueOf("MANAGE_CHANNELS"))) {
                    Server server = slashCommandInteraction.getServer().get();
                    String stringe = slashCommandInteraction.getArgumentStringValueByName("word").get();
                    List<String> bads;
